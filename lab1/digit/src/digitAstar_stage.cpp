...
const struct position final_pos[][22] =
{
    {
        {0, 0},
        {0, 0}, {0, 1}, {0, 2}, {0, 3}, {0, 4},
...
struct node_cmp
{
    bool operator() (Node *a, Node *b)
    {
        if (a->fValue > b->fValue)
            return true;
        // if (a->fValue == b->fValue && a->gValue > b->gValue)
        //     return true;
        else
            return false;
    }
        ...
        for (int j = 0; j < 5; j++)
            if ((pos = node->state[i][j]) != 0)
            {
                if (sevenPos.x == -1 && pos == 7)
                {
                    ...
                    tmp += g_weight[stage][pos] * (abs(final_pos[stage][pos].x - i) + abs(final_pos[stage][pos].y - j));
...
std::priority_queue<Node *, std::vector<Node *>, node_cmp> emptyPQueue;
...
void search()
{
    ...
    while (!nodePQueue.empty())
    {
            ...
            )
        {
            ...
            if (!finish1)
            {
                printf("********************************\nFound state with 1.2.6.7 Finished!\n");
                print_node(now);
