...
#define STEP_COST 10
#define MAX_INDEX_LIMIT 100000000
#define MAX_D_LIMIT 1000
...
// IDA* Search
void search(Node *initNode)
{
    Node *now, *nextNode;
    int tot_index = 0, curr_index = 0, iter_index = 0;
    gbl_index = &tot_index;
    // IDA*
    int status, next_d_limit;
    int d_limit = initNode->fValue;
    
    while (d_limit < MAX_D_LIMIT)
    {
        curr_index = 0;
        iter_index++;
        next_d_limit = MAX_D_LIMIT;
        while (!nodePQueue.empty())
        {
            now = nodePQueue.top();
            nodePQueue.pop();
            // IDA*
            // printf("\nd_limit: %d\nnext_d_limit: %d\nd(s): %d\n",d_limit, next_d_limit, now->fValue);
            if (now->fValue > d_limit)
            {
                next_d_limit = next_d_limit > now->fValue ? now->fValue : next_d_limit;
            }
            else
            {
                ...
                {
                    minH = now->hValue;
                    printf("\nCurrent Iteration: %d\nTotal Index: %d\nCurrent Index: %d\n", iter_index, tot_index, curr_index);
                    print_node(now);
                }
                else if (curr_index % 100000 == 0)
                {
                    printf("\n-----NOT MINH-----     Current minH = %d\nCurrent Iteration: %d\nTotal Index: %d\nCurrent Index: %d\n", minH, iter_index, tot_index, curr_index);
                    print_node(now);
                }

                if (tot_index >= MAX_INDEX_LIMIT)
                {
                    printf("\n-----NOT MINH-----     Current minH = %d\nIndex: %d\n", minH, tot_index);
                    print_node(now);
                    time_t stop_time = clock();
